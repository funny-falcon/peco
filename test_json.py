...
string = seq(eat('"'), cut, push(many(alt(eat(r'[^"\\]'), escaped))), eat('"'))
...
array = group(seq(skip(r'\['), cut, list_of(value, skip(',')), skip(']')))
member = group(seq(scan(string), skip(':'), value))
obj = seq(skip('{'), cut, group(list_of(member, skip(','))), skip('}'), mkobj)
